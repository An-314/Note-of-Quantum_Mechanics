...
$
  mat(a_1, a_2, dots;)^TT=
  mat(braket(1, psi), braket(2, psi), dots;)^TT
$
...
$
$
  mat(Psi_011; Psi_010; Psi_(01-1)) = (sqrt(2) alpha^(5 / 2)) / pi^(3 / 4) e^(-alpha^2 r^2 / 2) mat(
    -1 / sqrt(2) r sin theta e^(i phi); r cos theta; -1 / sqrt(2) - r sin theta e^(-i phi)
  ) prop mat(-1 / sqrt(2)(x + i y); x; 1 / sqrt(2)(x - i y))
$
$
  mat(Phi_100; Phi_010; Phi_001) = (sqrt(2) alpha^(5 / 2)) / pi^(3 / 4) e^(-alpha^2 r^2 / 2) mat(x; y; z)
  Phi = S^* Psi, S = mat(- 1 / sqrt(2), 0, 1 / sqrt(2); - i / sqrt(2), 0, -i / sqrt(2); 0, 1, 0)
$
...
$
  hat(J)_z = hbar mat(1, 0, 0; 0, 0, 0; 0, 0, -1), hat(J)_x = (hat(J)_+ + hat(J)_-) / 2 = hbar / sqrt(2) mat(
    0, 1, 0; 1, 0, 1; 0, 1, 0
  )
$
