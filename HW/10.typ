...
#exercise(subname: [12.4])[
  自由电子在沿 $z$ 轴的均匀磁场中运动，并被限制在 $x-y$ 平面。Landau规范下定态波函数的 $y$ 因子为平面波，求定态中 $y$ 方向的平均机械速度。
]
#solution[
  取电子电荷为 $-e$、Landau规范 $bold(A)=(0,B x,0)$，并令 $psi=exp(i k_y y)phi_n(x-x_0)$。机械动量为 $Pi_y=p_y+e B x$，谐振子中心满足 $x_0=-hbar k_y/(e B)$，故
  $
...
#exercise(subname: [12.5])[
  分别在对称规范和Landau规范下求电子机械动量 $Pi_x,Pi_y$ 的对易关系，并说明其是否依赖规范。
]
#solution[
  ...
  $
  对称规范 $bold(A)=(-B y/2,B x/2,0)$ 与Landau规范 $bold(A)=(0,B x,0)$ 均给出同一结果。它只依赖磁场 $bold(B)=nabla times bold(A)$，因而与规范选择无关。
]

#exercise(subname: [12.6])[
  在上题磁场中再加入沿 $x$ 轴负向的均匀电场 $cal(E)$。在Landau规范下求定态能级、波函数以及平均机械速度。
]
#solution[
  势能为 $e cal(E)x$。令 $psi=exp(i k_y y)phi(x)$，Hamilton量化为平移谐振子。以 $omega_c=e B/m$，中心
  $
