...
#exercise(subname: [7.4])[
  判断并说明下列说法：非定态中所有平均值都随时间变化；定态中本征值的测量概率不变；Hamilton量中出现的物理量都是守恒量；中心势定态中角动量必有确定值；自由粒子定态中动量必有确定值；一维粒子能级从不简并；中心势束缚能级至少有 $2l+1$ 重简并。
]
#solution[
...
#exercise(subname: [7.6])[
  若 $U$ 为幺正矩阵，证明 $U^dagger,U^(-1),U^*,U^TT$ 也都是幺正矩阵。
]
#proof[
  由 $U^dagger U=U U^dagger=1$ 可知 $U^(-1)=U^dagger$，故前两者幺正。对该式取复共轭得 $U^TT U^*=1$，再取转置得 $U^*U^TT=1$，所以 $U^*$ 与 $U^TT$ 互为逆与伴随，二者也幺正。
]

#exercise(subname: [7.7])[
  将坐标表象的定态Schrödinger方程变换到动量表象。
]
#solution[
