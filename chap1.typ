...
量子力学是研究微观粒子运动规律的学科，微观粒子包括原子、分子、原子核、基本粒子等。

量子力学里对粒子的基本描述是*波函数*。

系统的行为由Schrödinger方程描述，Schrödinger方程的解称为波函数。系统的完整信息用它的波函数表述，通过波函数可以计算任意可观察量的可能值。在空间给定体积内找到一个粒子的概率正比于波函数的模方。对于同样制备的相同系统进行同样的量不一定给出同一结果，相反，结果分布在波函数描述的范围内。
...
*Kirchhoff定律*：在热力学平衡的条件下，物体的辐射本领和吸收本领之比$F(nu, T) = r(nu, T) / a(nu, T)$是一个只与频率和温度有关的函数，与物体的材料和形状无关。_一个物体的辐射本领越强，它的吸收本领也越强，反之亦然。_
...
$
这就是*Planck辐射公式*。其中$c$是光速，$h$是Planck常数，$k_B$是Bohr兹曼常数。
...
*Rayleigh散射*：光子与气体分子的弹性散射——入射和出射光子能量一样。
...
#align(center)[
  #table(
    ...
    [Rayleigh散射、光电效应], [Compton散射], [产生正负电子对],
  )
]
